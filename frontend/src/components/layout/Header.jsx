      ...
      <div className="w-full bg-white rounded-full px-5 py-2 shadow-md border border-[#E2E8F0] flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#FF5722] text-ink text-[8px] font-bold rounded-full border border-white flex items-center justify-center animate-pulse">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {isNotifOpen && (
            <div className="absolute right-0 top-12 w-80 bg-white border border-[#E2E8F0] rounded-xl shadow-2xl z-50 p-4 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div key={n.id} className={`p-2.5 bg-[#F8FAFC] border border-slate-100 rounded-xl hover:bg-slate-100/80 transition-colors ${n.type === 'warning' ? 'border-l-4 border-l-red-500' : ''} ${n.type === 'info' ? 'border-l-4 border-l-blue-500' : ''}`}>
                    ...
                    {n.action === 'start-pdm' && n.taskId && (
                      <button 
                        ...
                        className="mt-2 w-full text-center py-1 bg-amber-500 hover:bg-amber-600 text-ink rounded-md text-[10px] font-bold transition-colors"
                      >
              ...
              <div className="mt-3 pt-2 border-t border-slate-100 flex justify-between items-center">
                <span className="text-[10px] text-gray-500">Sistem Berjalan Normal</span>
                <button
          ...
          <div className="flex items-center space-x-3 pl-3 border-l border-[#E2E8F0]">
            {user ? (
              <div className="flex items-center gap-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-full px-3 py-1">
                <div className="w-6 h-6 rounded-full bg-[#0F172A] text-ink flex items-center justify-center text-[10px] font-extrabold uppercase">
                  {user.name ? user.name.charAt(0) : 'U'}
                </div>
                <div className="text-left hidden sm:block">
                  <p className="text-xs font-bold text-[#0F172A] leading-tight capitalize">{user.name}</p>
                  <p className="text-[9px] text-gray-500 leading-tight">NPK: {user.npk} &bull; {user.role}</p>
                </div>
                <button
                  onClick={() => setShowPasswordModal(true)}
                  className="p-1 text-gray-500 hover:text-blue-600 transition-colors ml-1"
              ...
              <button
                ...
                className="group relative flex items-center gap-2 px-5 py-2 text-xs font-bold text-ink rounded-full overflow-hidden shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setShowPasswordModal(false)} className="text-gray-500 hover:text-slate-600 text-lg font-display font-medium">×</button>
            </div>
            ...
            <form onSubmit={handleChangePassword} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full text-sm px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 bg-slate-50"
                  placeholder="••••••••"
                />
              </div>
              ...
              <div className="pt-2">
                <button 
                  ...
                  className="w-full py-2.5 bg-navy-600 text-white rounded-lg text-xs font-bold hover:bg-navy-950 transition-colors disabled:opacity-50"
                >
