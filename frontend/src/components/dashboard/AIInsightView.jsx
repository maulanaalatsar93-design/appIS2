  ...
  return (
    <div className="space-y-6 relative pb-12">
      {loading && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/70 backdrop-blur-sm rounded-xl">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-xs font-semibold text-industrial-muted">Mengagregasi Analytics Keandalan Inspeksi...</span>
          </div>
        </div>
      )}

      {/* HEADER BAR */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-card shadow-sm-subtle border border-industrial-border">
        <div>
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-industrial-navy text-white rounded-xl shadow-sm">
              <Sparkles size={20} />
            </div>
            <div>
              <h1 className="text-base md:text-lg font-bold text-industrial-text tracking-tight flex items-center gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* PLANT FILTER */}
          <div className="flex items-center gap-1.5 bg-industrial-background border border-industrial-border px-3 py-1.5 rounded-lg text-xs">
            <Factory size={14} className="text-industrial-navy" />
            <select
              value={selectedPlant}
              onChange={(e) => setSelectedPlant(e.target.value)}
              className="bg-transparent font-bold text-industrial-text outline-none cursor-pointer"
            >
              {PLANT_OPTIONS.map((p) => (
          ...
          {/* MONTH / YEAR FILTER */}
          <div className="flex items-center gap-2 bg-industrial-background border border-industrial-border p-1.5 rounded-lg text-xs">
            <Calendar size={14} className="text-industrial-navy ml-1" />
            <button 
              onClick={() => setSelectedMonth((prev) => (prev === 0 ? 11 : prev - 1))}
              className="p-1 hover:bg-white text-industrial-muted hover:text-industrial-navy rounded transition-colors"
            >
              <ChevronLeft size={14} />
            </button>
            ...
            <select
              ...
              className="bg-transparent font-bold text-industrial-text outline-none cursor-pointer border-l border-industrial-border pl-2"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Fleet Health Score */}
        <div className="bg-white p-4 rounded-card shadow-sm-subtle border border-industrial-border flex items-center justify-between">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-industrial-muted">Fleet Health Score</div>
            <div className="text-2xl font-bold text-industrial-text mt-1">
              {healthData?.fleetAverageScore || 100} <span className="text-xs font-normal text-industrial-muted">/ 100</span>
            </div>
        ...
        <div className="bg-white p-4 rounded-card shadow-sm-subtle border border-industrial-border flex items-center justify-between">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-industrial-muted">Top Bad Actor Equipment</div>
            <div className="text-lg font-bold text-industrial-text mt-1 truncate max-w-[150px]">
        ...
        <div className="bg-white p-4 rounded-card shadow-sm-subtle border border-industrial-border flex items-center justify-between">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-industrial-muted">Akar Masalah Dominan</div>
            ...
            <div className="text-[10px] text-industrial-muted mt-0.5">Rule Keyword Frequency</div>
          </div>
        ...
        <div className="bg-white p-4 rounded-card shadow-sm-subtle border border-industrial-border flex items-center justify-between">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-industrial-muted">Work Center Beban Tertinggi</div>
          ...
          <div className="w-11 h-11 rounded-xl bg-industrial-blue/10 text-industrial-blue flex items-center justify-center border border-industrial-blue/20">
            <Wrench size={22} />
          </div>
        </div>
      </div>
      ...
      {execDashboard && (
        <div className="bg-industrial-navy text-white p-5 rounded-card shadow-sm-subtle space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="col-span-1 lg:col-span-7 bg-white p-5 border border-industrial-border rounded-card shadow-sm-subtle flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] text-industrial-muted font-medium">Histori Rekomendasi Inspeksi</span>
          </div>
          ...
          {rankingData.length > 0 ? (
            ...
            <div className="py-16 text-center text-xs text-industrial-muted">Tidak ada data peralatan.</div>
          )}
        </div>

        {/* ROOT CAUSE CATEGORY DISTRIBUTION */}
        <div className="col-span-1 lg:col-span-5 bg-white p-5 border border-industrial-border rounded-card shadow-sm-subtle flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-industrial-text flex items-center gap-2">
          ...
          {rootCauseData.length > 0 ? (
            ...
            <div className="py-16 text-center text-xs text-industrial-muted">Tidak ada distribusi data.</div>
          )}
        </div>
      </div>

      {/* REPEATED FAILURE DETECTION & EXPLAINABLE INSIGHT ("WHY?") TABLE */}
      <div className="bg-white p-5 border border-industrial-border rounded-card shadow-sm-subtle space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {repeatedData.length === 0 ? (
            <div className="col-span-2 py-10 text-center text-xs text-industrial-muted">Tidak ada rekomendasi berulang yang terdeteksi.</div>
          ) : (
            repeatedData.map((item, idx) => (
              <div key={idx} className="p-4 bg-industrial-background border border-industrial-border rounded-xl space-y-2.5 relative hover:border-industrial-blue transition-colors">
                <div className="flex items-center justify-between border-b border-industrial-border pb-2">
                  <div>
                    <div className="font-bold text-industrial-text text-sm flex items-center gap-1.5">
                      <span>{item.equipmentNo}</span>
                      <span className="text-[11px] font-medium text-industrial-muted">({item.equipmentDescription || item.equipmentNo})</span>
                    </div>
                ...
                <div className="text-xs text-industrial-text">
                  <span className="font-semibold text-industrial-muted">Estimasi Akar Masalah Dominan: </span>
                  <span className="font-bold text-industrial-orange">{item.dominantRootCause}</span>
                </div>

                <div className="bg-white p-3 rounded-lg border border-industrial-border text-xs text-industrial-muted space-y-1">
                  <div className="font-bold text-industrial-text uppercase tracking-wider text-[10px]">Tindakan Rekomendasi Terarah:</div>
                ...
                <div className="pt-1 flex justify-end">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-industrial-navy hover:bg-industrial-text text-white rounded-lg text-xs font-semibold transition-colors shadow-sm"
                  >
      ...
      {healthData && healthData.assets && (
        <div className="bg-white p-5 border border-industrial-border rounded-card shadow-sm-subtle space-y-4">
          <div className="flex items-center justify-between border-b border-industrial-border pb-3">
            ...
            <span className="text-[10px] text-industrial-muted">Healthy (&gt;85) | Attention (70-84) | Warning (50-69) | Critical (&lt;50)</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-industrial-background text-industrial-text font-bold border-b border-industrial-border">
              ...
              <tbody className="divide-y divide-industrial-border">
                {healthData.assets.map((asset, idx) => (
                  <tr key={idx} className="hover:bg-industrial-background/60 transition-colors">
                    <td className="py-2.5 px-3 font-semibold text-industrial-text">{asset.equipmentNo}</td>
                    <td className="py-2.5 px-3 text-industrial-muted font-medium">{asset.plantName} ({asset.plantCode})</td>
                    <td className="py-2.5 px-3 text-industrial-muted">{asset.workCenter}</td>
                    <td className="py-2.5 px-3 text-center font-bold text-industrial-text">{asset.healthScore}</td>
      ...
      {selectedExplainable && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-industrial-border">
            {/* Modal Header */}
            ...
            <div className="p-5 space-y-4">
              <div className="bg-industrial-background border border-industrial-border p-3 rounded-xl space-y-1">
                <div className="text-xs text-industrial-text">
                  <span className="font-semibold text-industrial-muted">Pabrik: </span>
                  <span className="font-bold text-industrial-text">{selectedExplainable.plantName} ({selectedExplainable.plantCode})</span>
                </div>
                <div className="text-xs text-industrial-text">
                  <span className="font-semibold text-industrial-muted">Maintenance Workcenter: </span>
                  <span className="font-bold text-industrial-blue">{selectedExplainable.workCenter}</span>
                </div>
                <div className="text-xs text-industrial-text">
                  <span className="font-semibold text-industrial-muted">Equipment: </span>
                  <span className="font-bold text-industrial-text">{selectedExplainable.equipmentNo} - {selectedExplainable.equipmentDescription || selectedExplainable.equipmentNo}</span>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-100 p-3 rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm font-bold text-industrial-text mt-0.5">{selectedExplainable.dominantRootCause}</div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">Confidence Score</div>
                  <div className="text-lg font-bold text-industrial-blue font-poppins">{selectedExplainable.confidenceScore}%</div>
                </div>
              </div>

              <div>
                <div className="text-xs font-bold text-industrial-text uppercase tracking-wide mb-2 flex items-center gap-1.5">
                ...
                <ul className="space-y-2">
                  {selectedExplainable.explainability.reasons.map((reason, rIdx) => (
                    <li key={rIdx} className="p-2.5 bg-industrial-background border border-industrial-border rounded-lg text-xs text-industrial-text flex items-start gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-industrial-blue mt-1.5 shrink-0"></span>
                      <span>{reason}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="bg-industrial-background p-3 rounded-xl border border-industrial-border text-xs">
                <div className="font-bold text-industrial-text mb-1">Rincian Bukti (Evidence Summary):</div>
                <div className="grid grid-cols-3 gap-2 text-center pt-1">
                  <div className="bg-white p-2 rounded border border-industrial-border">
                    <div className="text-[10px] text-industrial-muted font-bold uppercase">Total Rekomendasi</div>
                    <div className="text-base font-bold text-industrial-text">{selectedExplainable.explainability.evidence.recommendationCount || selectedExplainable.explainability.evidence.workOrderCount}</div>
                  </div>
                  <div className="bg-white p-2 rounded border border-industrial-border">
                    <div className="text-[10px] text-industrial-muted font-bold uppercase">Notifikasi M7</div>
                  ...
                  <div className="bg-white p-2 rounded border border-industrial-border">
                    <div className="text-[10px] text-industrial-muted font-bold uppercase">Keyword Match</div>
            ...
            {/* Modal Footer */}
            <div className="p-3 bg-industrial-background border-t border-industrial-border flex justify-end">
              <button 
                onClick={() => setSelectedExplainable(null)}
                className="px-4 py-1.5 bg-industrial-navy hover:bg-industrial-text text-white text-xs font-semibold rounded-lg transition-colors"
              >
                Tutup
              </button>
            </div>
