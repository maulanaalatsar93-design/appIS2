    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl font-display font-bold text-ink">Buat Work Program</h1>
            <p className="text-sm text-gray-500">Rencanakan program, pilih tim & approver dari database personel</p>
          </div>
        </div>
        <button onClick={handleSubmit} disabled={saving}
          className="flex items-center space-x-2 bg-industrial-green hover:bg-emerald-600 text-ink px-5 py-2.5 rounded-lg text-sm font-semibold shadow-sm disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4 shadow-sm-subtle">
            ...
            <div>
              ...
              <textarea name="notes" value={form.notes} onChange={handleChange} rows={3} placeholder="Catatan tambahan..."
                className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-industrial-blue resize-none" />
            </div>
          </div>
                      ...
                      return (
                        <label key={avp.id} className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer transition-colors ${selected ? 'bg-blue-50 border-blue-300' : 'border-industrial-border hover:bg-slate-50'}`}>
                          <div>
                            ...
                            <p className="text-[10px] text-gray-500">{avp.position} — {avp.divisi?.nama_divisi}</p>
                          </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Selected Members */}
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md h-[320px] flex flex-col">
            ...
            <div className="p-3 flex-1 overflow-y-auto space-y-2 bg-industrial-background/20">
              {selectedMembers.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-500">
                  ...
                  <p className="text-sm text-gray-500 font-medium">Belum ada personel dipilih.</p>
                  <p className="text-xs mt-1">Pilih dari daftar di bawah.</p>
                </div>
              ) : selectedMembers.map(m => (
                <div key={m.man_power_id} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-3 shadow-sm-subtle">
                  <div className="flex items-center space-x-2 flex-1">
                    <div>
                      <div className="flex items-center space-x-2">
                        <p className="font-semibold text-sm text-ink">{m.name}</p>
                        {coordinatorId == m.man_power_id && (
                          <span className="px-1.5 py-0.5 bg-navy-600 text-white text-[9px] font-bold rounded">KOORDINATOR</span>
                        )}
                        {m.hasConflict && (
                          <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 text-[9px] font-bold rounded border border-amber-200">⚠ Konflik</span>
                        )}
                      </div>
                      <p className="text-[10px] text-gray-500">{m.position}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <button onClick={() => removeMember(m.man_power_id)} className="p-1.5 text-gray-500 hover:text-red-500 hover:bg-red-50 border border-slate-200 rounded-lg transition-colors">
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md">
            <div className="p-4 border-b border-gray-200 bg-slate-50 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">Data ditarik dari database ManPower. Status cuti/sakit otomatis terdeteksi.</p>
              </div>
              <div className="flex items-center space-x-2">
                {form.start_date && form.end_date && filteredMp.length > 0 && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-navy-600 hover:bg-navy-950 text-white rounded-lg text-xs font-semibold flex items-center space-x-1 shadow-sm transition-all"
                  >
            ...
            {availableManpower.length > 0 && (
              <div className="p-3 bg-white border-b border-gray-200 flex gap-3 items-center">
                ...
                <div className="relative">
                  ...
                  {isDivDropdownOpen && (
                    <div className="absolute right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-30 p-2 space-y-1 w-56">
                      <div className="flex items-center justify-between px-2 py-1 border-b border-slate-100 mb-1">
                        <span className="text-[10px] font-bold text-gray-500">Bagian / Divisi:</span>
                        <button
                          type="button"
                      ...
                      <div className="pt-1 border-t border-slate-100 text-right">
                        <button
                          ...
                          className="px-2.5 py-0.5 bg-navy-600 text-white text-[11px] font-semibold rounded"
                        >
                          Tutup
                        </button>
                      </div>
                    ...
                    return (
                      <div key={mp.id} onClick={() => !isSelected && addMember(mp)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10.5px] text-gray-500 mt-0.5">{mp.position} <span className="mx-1">•</span> <span className="font-semibold">{mp.divisi?.nama_divisi}</span></p>
                          {hasConflict && !isSelected && (
                        ...
                        <div className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center border transition-colors ml-2 shadow-sm ${isSelected ? 'bg-slate-200 border-slate-300 text-slate-500' : 'bg-white border-slate-200 hover:bg-industrial-blue hover:border-industrial-blue hover:text-ink text-gray-500'}`}>
