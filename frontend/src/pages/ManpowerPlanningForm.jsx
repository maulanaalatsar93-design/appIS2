    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-ink">Buat Rencana Manpower</h1>
            <p className="text-platinum-dark text-sm">Susun form penempatan dan alur persetujuan</p>
          </div>
        </div>
        <button
          onClick={handleSubmit}
          disabled={saving}
          className="flex items-center space-x-2 bg-success hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-white border border-platinum-dark rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-navy focus:ring-1 focus:ring-navy"
              ...
              {!formData.isUrgentBypass && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => addApprover('AVP')} className="text-xs text-navy hover:text-blue-700 font-medium">+ Tambah AVP</button>
                  </div>
                  ...
                  {approvers.map((appr, idx) => appr.role === 'AVP' && (
                    <div key={idx} className="flex items-center space-x-2">
                      <select 
                        ...
                        className="w-full bg-white border border-platinum-dark rounded-lg px-3 py-1.5 text-sm text-ink focus:outline-none focus:border-navy"
                      >
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-platinum-dark rounded-card overflow-hidden shadow-soft-card flex flex-col h-[400px]">
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-2 bg-platinum/30">
              {selectedMembers.length === 0 ? (
                ...
                selectedMembers.map(member => (
                  <div key={member.manPowerId} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white border border-platinum-dark rounded-xl shadow-sm-subtle gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-platinum-dark">{member.position}</p>
                    </div>
          ...
          <div className="bg-white border border-platinum-dark rounded-card overflow-hidden shadow-soft-card">
            <div className="p-4 border-b border-platinum-dark flex items-center justify-between bg-platinum">
              <div className="flex flex-col">
                 ...
                 <p className="text-[10px] text-platinum-dark mt-0.5">Daftar anggota ditarik langsung dari database Man Power, mendeteksi status cuti/sakit secara otomatis.</p>
              </div>
            ...
            {availableManpower.length > 0 && (
              <div className="p-3 bg-white border-b border-platinum-dark flex gap-3">
            ...
            <div className="p-4 max-h-[350px] overflow-y-auto bg-platinum/20">
              {loadingManpower ? (
                <div className="flex justify-center py-6"><Loader2 className="w-6 h-6 animate-spin text-navy" /></div>
              ) : availableManpower.length === 0 ? (
                    ...
                    return (
                      <div key={mp.id} onClick={() => !isSelected && addMember(mp)} className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-colors ${isSelected ? 'bg-blue-50/50 border-navy-soft opacity-50 cursor-not-allowed' : hasConflicts ? 'bg-amber-50/50 border-amber-200 hover:bg-amber-100' : 'bg-white border-platinum-dark hover:border-navy shadow-sm-subtle'}`}>
                        <div>
                          ...
                          <p className="text-[10px] text-platinum-dark">{mp.position} • <span className="font-medium text-slate-600">{mp.divisi.nama_divisi}</span></p>
