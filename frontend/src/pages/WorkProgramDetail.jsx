...
export default function WorkProgramDetail({ programId, onBack, onUpdated }) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-20">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="flex items-center space-x-3 flex-wrap gap-2">
              <h1 className="text-xl font-display font-bold text-ink">{program.title}</h1>
            ...
            <p className="text-gray-500 text-xs mt-1">Dibuat oleh <span className="font-medium">{program.created_by?.name}</span> • {new Date(program.createdAt).toLocaleString('id-ID')}</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          ...
          {canAssign && (
            <button onClick={() => setSubView('assignment')}
              className="flex items-center space-x-2 bg-emerald-600 hover:bg-emerald-500 text-ink px-4 py-2 rounded-lg text-sm font-semibold">
      ...
      {/* Status Pipeline */}
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm-subtle overflow-x-auto">
          ...
          {STATUS_PIPELINE.map((s, i) => {
            ...
            return (
              <React.Fragment key={s}>
                <div className={`flex flex-col items-center px-3 py-2 rounded-xl text-center ${isActive ? 'bg-industrial-blue text-ink' : isPast ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-slate-50 text-gray-500 border border-slate-200'}`}>
                  {isPast && <CheckCircle2 className="w-4 h-4 mb-0.5" />}
                  <span className="text-[10px] font-bold whitespace-nowrap">{s}</span>
                </div>
                {i < STATUS_PIPELINE.length - 1 && <ChevronRight className="w-4 h-4 text-gray-500 shrink-0" />}
              </React.Fragment>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md">
            ...
            <div className="p-5 grid grid-cols-2 md:grid-cols-3 gap-5">
              {[
                ...
                <div key={f.label}>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">{f.label}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-gray-500 text-xs uppercase tracking-wide bg-slate-50/50 border-b border-gray-200">
                  <tr>
                ...
                <tbody className="divide-y divide-industrial-border">
                  {program.members.map(m => (
                    <tr key={m.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs">{m.man_power?.position}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{m.man_power?.divisi?.nama_divisi}</td>
          ...
          {program.items && program.items.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="text-gray-500 text-xs uppercase bg-slate-50/50 border-b border-gray-200">
                    <tr>
                  ...
                  <tbody className="divide-y divide-industrial-border">
                    {program.items.map(item => (
                      <tr key={item.id} className="hover:bg-slate-50">
                        <td className="px-4 py-3 font-mono text-xs text-gray-500">{item.item_no || '—'}</td>
        ...
        <div className="space-y-5">
          ...
          {isVpOrAdmin && (
            <div className="bg-white border border-industrial-blue/30 rounded-lg p-4 shadow-md">
              ...
              <div className="mb-4">
                <label className="block text-xs font-semibold text-gray-500 mb-1">Set Prioritas Program:</label>
                <select 
                  value={program.priority || 'Normal'} 
                  onChange={(e) => handlePriorityChange(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm font-semibold text-ink focus:outline-none focus:border-industrial-blue"
                >
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">Beri Masukan / Komentar:</label>
                <textarea 
                  value={feedbackNotes} 
                  onChange={e => setFeedbackNotes(e.target.value)} 
                  rows={2}
                  placeholder="Ketik masukan untuk tim disini..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-ink focus:outline-none focus:border-industrial-blue mb-2" 
                />
                <button 
                  onClick={handleAddFeedback} 
                  disabled={handlingFeedback || !feedbackNotes.trim()}
                  className="w-full py-2 bg-navy-600 text-white text-xs font-semibold rounded-lg hover:bg-navy-950 flex items-center justify-center disabled:opacity-50"
                >
          ...
          {canOverrideApprove && (
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 shadow-sm-subtle">
              <h3 className="font-semibold text-indigo-800 mb-3 text-sm flex items-center">
                <AlertTriangle className="w-4 h-4 mr-1.5" />
                VP / Admin Override Approval
              </h3>
              <p className="text-xs text-navy-600 mb-3 font-medium">
              ...
              <textarea value={approvalNotes} onChange={e => setApprovalNotes(e.target.value)} rows={2}
                ...
                className="w-full bg-white border border-indigo-200 rounded-lg p-2.5 text-xs text-ink focus:outline-none mb-3" />
              <div className="flex space-x-2">
                <button onClick={() => handleOverrideApproval('Approved')} disabled={processing}
                  className="flex-1 py-2 bg-emerald-600 text-ink text-xs font-semibold rounded-lg hover:bg-emerald-500 flex items-center justify-center">
                  <Check className="w-3.5 h-3.5 mr-1" /> Setuju
                </button>
                <button onClick={() => handleOverrideApproval('Revision')} disabled={processing}
                  className="flex-1 py-2 bg-amber-500 text-ink text-xs font-semibold rounded-lg hover:bg-amber-400">
                  Revisi
                </button>
                <button onClick={() => handleOverrideApproval('Rejected')} disabled={processing}
                  className="flex-1 py-2 bg-red-500 text-ink text-xs font-semibold rounded-lg hover:bg-red-400">
                  Tolak
                </button>
              </div>
            </div>
          ...
          {pendingApprovals.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 shadow-sm-subtle">
                ...
                {pendingApprovals.map(a => (
                  <label key={a.id} className={`flex items-center justify-between p-2 rounded-xl border cursor-pointer ${selectedApproverId === a.approver_mp_id ? 'bg-blue-100 border-blue-400' : 'bg-white border-blue-200'}`}>
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">{a.approver?.position} ({a.level})</p>
                    </div>
              ...
              <textarea value={approvalNotes} onChange={e => setApprovalNotes(e.target.value)} rows={2}
                placeholder="Catatan (wajib untuk Revision/Reject)..."
                className="w-full bg-white border border-blue-200 rounded-lg p-2.5 text-xs text-ink focus:outline-none mb-3" />
              <div className="flex space-x-2">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-md">
            ...
            <div className="space-y-3">
              {program.approvals.length === 0 ? (
                <p className="text-xs text-gray-500 italic">Tidak ada approver (Bypass VP otomatis).</p>
                ...
                <div key={a.id} className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                  ...
                  {a.notes && <p className="text-[10px] text-gray-500 mt-1.5 bg-white p-2 rounded border border-slate-100 italic flex items-start"><MessageSquare className="w-3 h-3 mr-1 shrink-0 mt-0.5" />{a.notes}</p>}
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Audit Trail */}
          <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-md max-h-72 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              {program.audits.map(a => (
                <div key={a.id} className="relative pl-5 border-l-2 border-slate-200">
                  ...
                  <p className="text-[10px] text-gray-500">{new Date(a.createdAt).toLocaleString('id-ID')} • {a.actor?.name}</p>
                  {a.details && <p className="text-[10px] text-gray-500 mt-0.5 bg-slate-50 p-1.5 rounded">{a.details}</p>}
                </div>
              ))}
            </div>
          </div>
