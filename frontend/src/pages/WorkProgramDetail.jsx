...
export default function WorkProgramDetail({ programId, onBack, onUpdated }) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-20">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="flex items-center space-x-3 flex-wrap gap-2">
              <h1 className="text-xl font-bold text-industrial-text">{program.title}</h1>
              ...
              {program.priority && (
                <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border ${
                  ...
                  program.priority === 'Normal' ? 'bg-blue-50 text-blue-600 border-blue-200' :
            ...
            <p className="text-industrial-muted text-xs mt-1">Dibuat oleh <span className="font-medium">{program.created_by?.name}</span> • {new Date(program.createdAt).toLocaleString('id-ID')}</p>
          </div>
        </div>
      ...
      {/* Status Pipeline */}
      <div className="bg-white border border-industrial-border rounded-card p-4 shadow-sm-subtle overflow-x-auto">
          ...
          {STATUS_PIPELINE.map((s, i) => {
            ...
            return (
              <React.Fragment key={s}>
                <div className={`flex flex-col items-center px-3 py-2 rounded-xl text-center ${isActive ? 'bg-industrial-blue text-white' : isPast ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-slate-50 text-slate-400 border border-slate-200'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white border border-industrial-border rounded-card overflow-hidden shadow-soft-card">
            <div className="p-4 border-b border-industrial-border bg-slate-50">
              <h3 className="font-semibold text-industrial-text">Detail Program</h3>
            </div>
            <div className="p-5 grid grid-cols-2 md:grid-cols-3 gap-5">
              {[
                ...
                <div key={f.label}>
                  <p className="text-[10px] font-bold text-industrial-muted uppercase tracking-wider mb-1">{f.label}</p>
                  <p className="text-sm font-semibold text-industrial-text">{f.value || '—'}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white border border-industrial-border rounded-card overflow-hidden shadow-soft-card">
            <div className="p-4 border-b border-industrial-border bg-slate-50">
              <h3 className="font-semibold text-industrial-text">Tim Program ({program.members.length} orang)</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-industrial-muted text-xs uppercase tracking-wide bg-slate-50/50 border-b border-industrial-border">
                  <tr>
                ...
                <tbody className="divide-y divide-industrial-border">
                  {program.members.map(m => (
                    <tr key={m.id} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-semibold text-industrial-text">{m.man_power?.name}</td>
                      <td className="px-4 py-3 text-industrial-muted text-xs">{m.man_power?.position}</td>
                      <td className="px-4 py-3 text-industrial-muted text-xs">{m.man_power?.divisi?.nama_divisi}</td>
                      <td className="px-4 py-3">
                        <span className="text-xs font-semibold text-industrial-blue">{m.role || 'Member'}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {program.items && program.items.length > 0 && (
            <div className="bg-white border border-industrial-border rounded-card overflow-hidden shadow-soft-card">
              <div className="p-4 border-b border-industrial-border bg-slate-50">
                <h3 className="font-semibold text-industrial-text">Work Items ({program.items.length})</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="text-industrial-muted text-xs uppercase bg-slate-50/50 border-b border-industrial-border">
                    <tr>
                  ...
                  <tbody className="divide-y divide-industrial-border">
                    {program.items.map(item => (
                      <tr key={item.id} className="hover:bg-slate-50">
                        <td className="px-4 py-3 font-mono text-xs text-industrial-muted">{item.item_no || '—'}</td>
                        <td className="px-4 py-3 font-semibold text-industrial-text">{item.title}</td>
                        <td className="px-4 py-3 text-xs text-industrial-muted">{item.equipment || '—'}</td>
                        <td className="px-4 py-3 text-xs text-industrial-text">{item.pic?.name || '—'}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 text-[10px] font-semibold rounded-full border ${item.status === 'Done' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : item.status === 'In Progress' ? 'bg-blue-50 text-blue-700 border-blue-200' : item.status === 'Ready For Review' ? 'bg-amber-50 text-amber-700 border-amber-200' : item.status === 'Hold' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                            {item.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
        ...
        <div className="space-y-5">
          ...
          {isVpOrAdmin && (
            <div className="bg-white border border-industrial-blue/30 rounded-card p-4 shadow-soft-card">
              <h3 className="font-semibold text-industrial-text mb-3 text-sm flex items-center">
                <Activity className="w-4 h-4 mr-1.5 text-industrial-blue" />
                Panel Atasan (Monitoring & Feedback)
              </h3>
              
              <div className="mb-4">
                <label className="block text-xs font-semibold text-industrial-muted mb-1">Set Prioritas Program:</label>
                <select 
                  value={program.priority || 'Normal'} 
                  onChange={(e) => handlePriorityChange(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm font-semibold text-industrial-text focus:outline-none focus:border-industrial-blue"
                >
              ...
              <div>
                <label className="block text-xs font-semibold text-industrial-muted mb-1">Beri Masukan / Komentar:</label>
                <textarea 
                  value={feedbackNotes} 
                  onChange={e => setFeedbackNotes(e.target.value)} 
                  rows={2}
                  placeholder="Ketik masukan untuk tim disini..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-industrial-text focus:outline-none focus:border-industrial-blue mb-2" 
                />
                <button 
                  onClick={handleAddFeedback} 
                  disabled={handlingFeedback || !feedbackNotes.trim()}
                  className="w-full py-2 bg-industrial-blue text-white text-xs font-semibold rounded-lg hover:bg-blue-700 flex items-center justify-center disabled:opacity-50"
                >
          ...
          {canOverrideApprove && (
            <div className="bg-indigo-50 border border-indigo-200 rounded-card p-4 shadow-sm-subtle">
              ...
              <textarea value={approvalNotes} onChange={e => setApprovalNotes(e.target.value)} rows={2}
                ...
                className="w-full bg-white border border-indigo-200 rounded-xl p-2.5 text-xs text-industrial-text focus:outline-none mb-3" />
          ...
          {pendingApprovals.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-card p-4 shadow-sm-subtle">
              ...
              <div className="space-y-2 mb-3">
                ...
                {pendingApprovals.map(a => (
                  <label key={a.id} className={`flex items-center justify-between p-2 rounded-xl border cursor-pointer ${selectedApproverId === a.approver_mp_id ? 'bg-blue-100 border-blue-400' : 'bg-white border-blue-200'}`}>
                    <div>
                      <p className="text-xs font-semibold text-industrial-text">{a.approver?.name}</p>
                      <p className="text-[10px] text-industrial-muted">{a.approver?.position} ({a.level})</p>
                    </div>
              ...
              <textarea value={approvalNotes} onChange={e => setApprovalNotes(e.target.value)} rows={2}
                placeholder="Catatan (wajib untuk Revision/Reject)..."
                className="w-full bg-white border border-blue-200 rounded-xl p-2.5 text-xs text-industrial-text focus:outline-none mb-3" />
              <div className="flex space-x-2">
          ...
          {/* Approval Status */}
          <div className="bg-white border border-industrial-border rounded-card p-4 shadow-soft-card">
            <h3 className="font-semibold text-industrial-text mb-3">Status Persetujuan</h3>
              ...
              {program.approvals.length === 0 ? (
                ...
                <div key={a.id} className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-industrial-text">{a.approver?.name}</p>
                      <p className="text-[10px] text-industrial-muted">{a.approver?.position} • {a.level}</p>
                    </div>
                    <span className={`px-2 py-0.5 text-[10px] font-bold rounded border ${a.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : a.status === 'Rejected' ? 'bg-red-50 text-red-600 border-red-200' : a.status === 'Revision' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                      {a.status}
                    </span>
                  </div>
          ...
          {/* Audit Trail */}
          <div className="bg-white border border-industrial-border rounded-card p-4 shadow-soft-card max-h-72 flex flex-col">
            <h3 className="font-semibold text-industrial-text mb-3">Jejak Aktivitas</h3>
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              {program.audits.map(a => (
                <div key={a.id} className="relative pl-5 border-l-2 border-slate-200">
                  ...
                  <p className="text-xs font-bold text-industrial-text">{a.action}</p>
                  <p className="text-[10px] text-industrial-muted">{new Date(a.createdAt).toLocaleString('id-ID')} • {a.actor?.name}</p>
                  {a.details && <p className="text-[10px] text-slate-500 mt-0.5 bg-slate-50 p-1.5 rounded">{a.details}</p>}
                </div>
              ))}
            </div>
          </div>
